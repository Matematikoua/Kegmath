   ...
   <mfenced open="(" close=")">
    <mtable>
      ...
      },
      {
        ...
        url: "https://drive.google.com/file/d/1qo_QcycwwNUwsFikksl-3fNT5J7LlVpc/view?usp=sharing",
    ...
    return (
      <section className="py-20 bg-white" data-name="material-catalog" data-file="components/Materialdidactico.js">
            ...
            {materiales.map(material => (
              <div key={material.id} className="card">
                <div className="w-16 h-16 bg-[var(--secondary-color)] bg-opacity-10 rounded-xl flex items-center justify-center mb-4">
                  <div className={`text-3xl text-[var(--primary-color)] `}><mn>{material.icon}</mn></div>
                </div>
                <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mb-3 ${getLevelColor(material.level)}`}>
                  {material.level}
                </span>
                <h3 className="text-xl font-bold mb-3 text-[var(--primary-color)]">{material.title}</h3>
                <p className="text-[var(--text-secondary)] mb-4 text-sm">{material.description}</p>
                <div className="flex items-center text-sm text-[var(--text-secondary)] mb-4">
                  <div className="icon-clock text-base mr-2"></div>
                  {material.duration}
                </div>
                <a href={material.url} className="btn-primary w-full text-center">
                  Acceder al material 
                </a>
              </div>
            ))}
    ...
    console.error('Materialdidactico component error:', error);
    return null;
  }
}